...
def q_network(input_tensor, scope_name, input_length=512, num_action=11,
              duel=False, batch_norm=False, phase_train=None):
    with tf.variable_scope(scope_name):
        ...
        if duel:
            ...
            # rate=0.0 means no dropout (replaces keep_prob=1.0, which is deprecated positional use)
            v_tensor = tf.nn.dropout(input_tensor, rate=0.0)
            ...
            tensor = tf.nn.dropout(input_tensor, rate=0.0)
            ...
            q = v_tensor + (tensor - tf.reduce_mean(tensor, axis=1, keepdims=True))
            return q, weights
            ...
            weights = {
                ...
                'fc4_weights': fc4_weights, 'fc4_biases': fc4_biases,
            }
...
def q_network_shallow(input_tensor, scope_name, input_length=512, num_action=11, duel=False):
    with tf.variable_scope(scope_name):
        ...
        if duel:
            v_fc1_weights = tf.get_variable("v_fc1_weights", [input_length, 512], initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / 512)))
            v_fc1_biases  = tf.get_variable("v_fc1_biases",  [512],              initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / 512)))
            v_fc2_weights = tf.get_variable("v_fc2_weights", [512, 1],            initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / 512)))
            v_fc2_biases  = tf.get_variable("v_fc2_biases",  [1],               initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0)))

            weights = {
                'v_fc1_weights': v_fc1_weights, 'v_fc1_biases': v_fc1_biases,
                'v_fc2_weights': v_fc2_weights, 'v_fc2_biases': v_fc2_biases,
            }

            v_tensor = tf.nn.dropout(input_tensor, rate=0.0)
            v_tensor = tf.nn.relu(tf.matmul(v_tensor, v_fc1_weights) + v_fc1_biases)
            v_tensor = tf.matmul(v_tensor, v_fc2_weights) + v_fc2_biases

            fc1_weights = tf.get_variable("fc1_weights", [input_length, 512], initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / 512)))
            fc1_biases  = tf.get_variable("fc1_biases",  [512],              initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / 512)))
            fc2_weights = tf.get_variable("fc2_weights", [512, num_action],   initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / 512)))
            fc2_biases  = tf.get_variable("fc2_biases",  [num_action],       initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / num_action)))
            ...
            tensor = tf.nn.dropout(input_tensor, rate=0.0)
            tensor = tf.nn.relu(tf.matmul(tensor, fc1_weights) + fc1_biases)
            tensor = tf.matmul(tensor, fc2_weights) + fc2_biases
            ...
            fc1_weights = tf.get_variable("fc1_weights", [input_length, 512], initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / 512)))
            fc1_biases  = tf.get_variable("fc1_biases",  [512],              initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / 512)))
            fc2_weights = tf.get_variable("fc2_weights", [512, 512],          initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / 512)))
            fc2_biases  = tf.get_variable("fc2_biases",  [512],              initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / 512)))
            fc3_weights = tf.get_variable("fc3_weights", [512, 512],          initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / 512)))
            fc3_biases  = tf.get_variable("fc3_biases",  [512],              initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / 512)))
            fc4_weights = tf.get_variable("fc4_weights", [512, num_action],   initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / 512)))
            fc4_biases  = tf.get_variable("fc4_biases",  [num_action],       initializer=tf.truncated_normal_initializer(stddev=np.sqrt(1.0 / num_action)))
    ...
    with tf.variable_scope(scope_name):
        ...
        if duel:
            ...
            weights = {
                'v_fc1_weights': v_fc1_weights, 'v_fc1_biases': v_fc1_biases,
                'v_fc2_weights': v_fc2_weights, 'v_fc2_biases': v_fc2_biases,
                'v_fc3_weights': v_fc3_weights, 'v_fc3_biases': v_fc3_biases,
                'v_fc4_weights': v_fc4_weights, 'v_fc4_biases': v_fc4_biases,
                'v_fc5_weights': v_fc5_weights, 'v_fc5_biases': v_fc5_biases,
            }
            ...
            weights.update({
                ...
                'fc3_weights': fc3_weights, 'fc3_biases': fc3_biases,
                'fc4_weights': fc4_weights, 'fc4_biases': fc4_biases,
                'fc5_weights': fc5_weights, 'fc5_biases': fc5_biases,
            })
